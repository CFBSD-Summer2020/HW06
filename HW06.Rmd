...
H3K27M <- sapply(H3K27M_files, read.tsv) %>%
  select(contains("TPM"), Name) %>%
  rename(gene_id = Name)

WT <- sapply(WT_files, read.tsv) %>%
  select(contains("TPM"))

Name <- read.tsv(H3K27M) %>%
  select(gene_id)

GBM.transcripts <- rbind(H3K27M, WT)
...
##I'm unable to install two of the three packages? I made sure I have the most up to date version of R studio but it's still telling me that my current version of R doesn't support the packages.
```
...
genes_of_interest <- c("IRX1", "OSR1", "DCHS2", "BRINP3", "TOB2P1", "FOXD1", "ZFPM2", "GLB1", "ALG5", "TRIM4", "ADARB2", "PCDHGA11", "IDH1", "EGFR", "MGMT", "TERT", "PTEN", "TP53", "RB1", "PTEN", "ATRX", "PDGFRA", "PIK3CA", "MICA", "CDKN2A", "EZH2", "BRD2", CEP72, HSF1)
