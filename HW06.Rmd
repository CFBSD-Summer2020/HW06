...
```{r test}
read_tsv("~/HW06/RNA_Seq_processed/H3K27M/DIPGVI_C30.sf")
...
```{r map gene.ids to gene.symbols using ensembldb, cache=TRUE}
install.packages("BiocManager")
BiocManager::install("ensembldb")
BiocManager::install("EnsDb.Hsapiens.v75")
```

```{r mapping continued, cache=TRUE}
library(ensembldb)
...
```{r t.test for difference between a bunch of genes, cache=TRUE}
...
GBM.genes.of.interest<- GBM.genes.of.interest %>%
  ...
  spread(gene_symbol, tpm) 

GBM.genes.of.interest$sample[1:23]<- "H3K27M"
GBM.genes.of.interest$sample[24:45]<- "WT"

for (gene in genes_of_interest) {
gene_select <- GBM.genes.of.interest %>% 
  select(sample, gene)
t.test(formula= gene_select$gene~ gene_select$sample, data= gene_select)}

#t.test(subset(gene_select$gene,gene_select$sample == "H3K27M")~ subset(gene_select$gene,gene_select$sample == "WT"))
```
