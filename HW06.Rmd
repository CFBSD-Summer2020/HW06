...
for(file in 1:length(H3K27M_filepath_list)) {
  assign(x = paste("RNA_H3K", H3K27M_filename_list[[file]], sep = "_"),
         value = read_tsv(H3K27M_filepath_list[[file]]))
} 

for(file in 1:length(WT_filepath_list)) {
  assign(x = paste("RNA_WT", WT_filename_list[[file]], sep = "_"),
         value = read_tsv(WT_filepath_list[[file]]))
} 
#Sorry about the readability of this for loop. The assign() function allows for the for loop to create a new dataframe for each interation of the loop. The x argument is the name that changes for each location in H3K27M_filepath_list - I created a vector called H3K27M_filename_list so that the new dataframe name is more readable. The value argument of assign() is what variable x is being assigned to.

#To put it more simply, the x argument represents the left side of the <- when we typically assign objects, while the value argument represents the right side.
...
for(file in 1:length(H3K27M_filepath_list)) {
  H3k_RNA_list[[file]] <- assign(x = paste("RNA_H3K", H3K27M_filename_list[[file]], sep = "_"),
                                value = read_tsv(H3K27M_filepath_list[[file]]))
}
...
for(file in 1:length(H3K_RNA_list)) {
  H3K_RNA_list[[file]] %>% 
  select(Name, TPM) %>% 
  inner_join(H3K_RNA_list[[file + 1]], by = "Name")
