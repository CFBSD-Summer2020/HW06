...
#made list of wt file names
wt_files <- list.files(path = "RNA_Seq_processed/WT", pattern = "*.sf") 

#load, combine, and tidy data
wt <- sapply(paste("RNA_Seq_processed/WT/", wt_files, sep = ""), read.table, header = TRUE, simplify = FALSE) %>% #read in all files
...
#rename columns
colnames(wt) <- c("gene_id", wt_files)


#repeat with H3K27M
H3K27M_files <- list.files(path = "RNA_Seq_processed/H3K27M", pattern = "*.sf") 

H3K27M <- sapply(paste("RNA_Seq_processed/H3K27M/", H3K27M_files, sep = ""), read.table, header = TRUE, simplify = FALSE) %>% 
  bind_rows(.id = "id") %>%
  select("Name", "id", "TPM") %>% 
  spread("id", "TPM") 

colnames(H3K27M) <- c("gene_id", H3K27M_files)
