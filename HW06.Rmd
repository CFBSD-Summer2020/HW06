...
#It looks like one of these genes of interest in not in the GBM.genes.of.interest data frame. I think that is causing potential problems later on.
...
for(gene in 1:length(genes_of_interest)) {
  if(gene == 26) { #This is due to the test counting the column names as data, so I had to shift the for() loop down - meaning I also had to cut it short
    break
  }
t_test <- tidy(t.test(x = GBM.genes.of.interest[gene + 1, 2:24],
                      y = GBM.genes.of.interest[gene + 1, 25:46])) %>% 
  bind_rows(t_test)
}
#print out the t-test results
tidy(t.test(x = GBM.genes.of.interest[26, 2:24],
            y = GBM.genes.of.interest[26, 25:46]))
```
