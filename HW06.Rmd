...
#made list of file names
all_files <- list.files(path = "RNA_Seq_processed", full.names = TRUE, recursive = TRUE, pattern = "*.sf") 
  #thank you Alex and Sherzod for the full.names and recursive arguments, my code was an absolute mess and twice as long before this :) <3

#load, combine, and tidy data
GBM.transcripts <- sapply(all_files, read.table, header = TRUE, simplify = FALSE) %>% #read in all files
  bind_rows(.id = "id") %>% #combine and mark original set name
  select("Name", "id", "TPM") %>% #select only gene name, set name, and TPM
  spread("id", "TPM") #spread TPM by id
#final product: dataframe, 1 col for names, 1 TPM col per file

#rename columns
colnames(GBM.transcripts) <- c("gene_id", all_files)
...
#added: TWIST1, SNAI1, and SOX4
genes_of_interest <- c("IRX1", "OSR1", "DCHS2", "BRINP3", "TOB2P1", "FOXD1", "ZFPM2", "GLB1", "ALG5", "TRIM4", "ADARB2", "PCDHGA11", "IDH1", "EGFR", "MGMT", "TERT", "PTEN", "TP53", "RB1", "PTEN", "ATRX", "PDGFRA", "PIK3CA", "MICA", "CDKN2A", "EZH2", "BRD2", "TWIST1", "SNAI1", "SOX4")
...
#WT samples are found col 25 - 46, H3K 2 - 24
for(row in 1:nrow(GBM.genes.of.interest)){
  print(t.test(GBM.genes.of.interest[row,25:46], GBM.genes.of.interest[row,2:24]))
}
...
```
This is where I get stuck. I'm not sure how to store my t-test output into a dataframe to use for the next section. I saw on stack overflow I could use magicfor, but when I try to load the package it tells me it doesn't exist. I'm also not sure how to approach the rest.
